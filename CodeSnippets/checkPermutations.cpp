...
The time complexity may be represented by the number of comparisons required
based on N, the length of the strings.

As a string of length N will in the worst case require a quantity of comparisons
matching the number of permutations available, the pattern will be as follows:
...
However, this only represents the number of string comparisons to perform.
As such comparisons are char-by-char comparisons down the string, we may expect
a time complexity closer to:
...
1) If the string size is an O(1) accessible field, it may be checked first.
  - Any strings with differing lengths may bypass comparisons
...
Which simplifies to a time complexity of:
O( n )
...
For each character in string1
  Add 1 to the appropriate cell in charCounts
For each character in string2
  Subtract 1 from the appropriate cell in charCounts
If any of the cells is non-zero
    Return false
...
*/
// Compile with:
// $ g++ --std=c++11 01_02_checkPermutations.cpp -o checkPermutations
// Run with:
// $ ./checkPermutations
...
bool checkPermutations( std::string string1, std::string string2 ) {
   ...
   for ( int i = 0 ; i < string2.length( ) ; i++ ) {
      ...
      // If the result is <0, then more of a character exist in string 2 than string 1
