...
node. Traverse the list again this number of times and remove the node.
...
While using a runner technique may seem like an optimization:
  ...
  The trailing runner indicates the appropriate node to remove.
...
The removal may be completed in a time complexity of:
...
// (+) --------------------------------|
// #removeKthToLast( LinkedList, int )
// ------------------------------------|
...
// RetVal:  None
void removeKthToLast( LinkedList tgtList, int K ) {
